...
def sc2_pcr(source: np.ndarray, target: np.ndarray, visualize=False):
    ...
    matcher = Matcher(inlier_threshold=cfg.inlier_threshold,
    ...
    src_keypts = torch.from_numpy(A_xyz).unsqueeze(0).cuda().transpose(1, 2).float()
    tgt_keypts = torch.from_numpy(B_xyz).unsqueeze(0).cuda().transpose(1, 2).float()
    src_features = torch.from_numpy(A_feats).unsqueeze(0).cuda()
    tgt_features = torch.from_numpy(B_feats).unsqueeze(0).cuda()
